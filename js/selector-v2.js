...
const EVENTO_SLUG   = 'boda-rocio-jose';
...
// ========================================
// GLOBAL VARIABLES - Boda Rocio y Jose
// ========================================
// Lista de fotos generada automáticamente
// Total: 1111 fotos
// Generado: 2026-03-29 03:11
const photos = [];  // Se llenara despues del evento (4 dic 2026) con generate_photos_array.py
...
const CONFIG = {
    slug:               'boda-rocio-jose',
    nombre:             (window.EVENT_CONFIG && window.EVENT_CONFIG.nombre)             || 'Rocio y Jose',
    telefono:           (window.EVENT_CONFIG && window.EVENT_CONFIG.telefono)           || '',
    fechaEvento:        (window.EVENT_CONFIG && window.EVENT_CONFIG.fechaEvento)        || new Date(2026, 11, 4, 17, 0, 0),
    limiteImpresion:    200,
    limiteInvitacion:   null,
    costoFotoAdicional: (window.EVENT_CONFIG && window.EVENT_CONFIG.costoFotoAdicional) || 15,
};

const STORAGE_KEY = 'boda_rocio_jose_photo_selections';
const KEY_FILTER   = 'brj_filter';
const KEY_SCROLL   = 'brj_scroll';
const KEY_LAST     = 'brj_last_photo';
const LIMITES = {
...
async function sbSyncSelections() {
    ...
    const evento_id = await sbGetEventoId();
    if (!evento_id) return;
    // El trigger reject_old_code_version descarta la fila EN SILENCIO (201 sin
    // guardar) si falta code_version >= 3 o _sync.clock no crece. Date.now() es
    // monotono, asi que siempre supera al reloj almacenado.
    ...
    const rows = Object.entries(snapshot).map(([idx, sel]) => ({
        ...
        code_version: 6,
        datos: {
            impresion: sel.impresion || false, ampliacion: false, invitacion: sel.invitacion || false,
            descartada: sel.descartada || false, notes: '',
            _sync: { clock, sid: SESSION_ID, updatedAt: new Date().toISOString(), deleted: false }
        }
    }));
...
function exportToJSON() {
    ...
    a.href = url;
    a.download = `seleccion-fotos-boda-rocio-jose-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
...
function generateTextSummary() {
    ...
    let summary = '🎉 SELECCIÓN DE FOTOS - XV AÑOS CLAUDIA CAROLINA VALDENEGRO TORRES\n';
    summary += '═══════════════════════════════════════════════════\n\n';
